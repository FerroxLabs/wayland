...
description: |
  Produces a short-form video plan for TikTok, Instagram Reels, or YouTube
...
- The user needs a full script with dialogue, scene descriptions, and shot-by-shot coverage for videos longer than 60 seconds -- use `video-script-writing` instead
- The user needs a YouTube channel growth strategy, upload cadence, or metadata optimization beyond a single video caption -- use `youtube-video-strategy` instead
- The user needs a detailed shot list with camera angles, lens choices, lighting setups, and editing sequences -- use `video-storyboard` instead
...
7. **Loop optimization is not optional.** Every plan must specify all three loop dimensions: visual, audio, and semantic. A video plan without a loop design is an incomplete plan. The loop is a compounding return -- a 15-second video watched 3 times generates 45 seconds of watch time. The algorithm does not differentiate between one watch of 45 seconds and three watches of 15 seconds.

8. **Never plan an ending that signals the video is over.** Specific endings that must never appear in a short-form video plan: "Thanks for watching," fade to black, outro music that is different from the main audio, "Like and subscribe," or any verbal statement that uses the word "end" or "done" in a closing way. These endings trigger the viewer's "I should go" response and break the loop potential.

9. **The caption's above-fold line must stand alone as complete communication.** Test it by reading only that line with no other context -- does it make a complete statement or create a compelling question? If it requires the body text to make sense, rewrite it. At least 60% of viewers who see the caption will only read the above-fold line.

10. **Trending audio plans must include a shelf-life note.** If the plan incorporates a trending sound, it must note that trending sounds on TikTok have a peak window of approximately 2-3 weeks from initial trending, after which the sound becomes associated with "old content" by the algorithm's freshness weighting. The plan should flag whether the trend is in its rising phase (use it now), peak phase (use within 1 week), or declining phase (consider alternative audio).

11. **Duration selection should default to the minimum duration that fully delivers the concept.** A concept that can be delivered in 20 seconds should not be padded to 45 seconds with filler. Padding is any audio or visual that does not advance the core message or deepen engagement. Short completion rates from a shorter video outperform partial completion rates from a longer video. Always justify a duration over 30 seconds with a specific reason (complexity, step count, story arc requirement).

12. **Product or promotional content must lead with value for the first 70% of the video.** The product name, brand, or purchase CTA should not appear before the 70% timestamp. Opening with a product pitch triggers the viewer's "ad response" -- an unconscious categorization of the content as not-worth-watching. The hook and early beats must be purely value-driven. The product or service appears as the solution to the problem established in the first 70%.
...
4. "1. Water 2. One priority 3. 5 min movement" -- Beat 4, list format, center frame
5. "Focus before inbox." -- Beat 5, large and clean, center frame
