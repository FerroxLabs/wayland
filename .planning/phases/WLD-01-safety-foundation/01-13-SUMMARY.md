...
provides:
  ...
  - Local-only OfficeCLI PATH authority with no Desktop fallback
affects: [01-24, 01-31]
...
- Shell PATH exposure now requires the same target-exact executable and authority manifests, rejects symlink substitution and existence-only manifests, and never consults npm/global/cache fallback executables.
