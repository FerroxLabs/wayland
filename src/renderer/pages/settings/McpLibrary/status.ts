 ...
 *
 * - reachable: enabled + standalone probe succeeded; chat readiness is unproven
 ...
 */
export type UIStatus = 'reachable' | 'warn' | 'error' | 'stopped';

/**
 * Derive the UI status from the stored server record + its live OAuth status.
 * error short-circuits first, then warn (needsLogin), then reachable
 * (enabled + probe-reachable), else stopped (absorbs disconnected / testing / undefined).
 */
export function deriveStatus(s: IMcpServer, oauth: McpOAuthStatus | undefined): UIStatus {
  ...
  if (s.enabled === true && s.status === 'connected') return 'reachable';
  return 'stopped';
}
