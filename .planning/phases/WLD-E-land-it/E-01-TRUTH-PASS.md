...
`tests/unit/coworkAuthorityIsolation.test.ts` — _"resolves no executable when the verified lockstep
capability is unavailable (fail closed)"_. It asserted `resolveVerifiedOfficecliCommand()` returns
...
**One low-priority flag, not a blocker:** the two `modelsSettings` fixtures are _shaped_ like real
