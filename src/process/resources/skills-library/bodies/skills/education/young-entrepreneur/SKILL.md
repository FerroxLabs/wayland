---
name: young-entrepreneur
description: |
  Age-appropriate guidance for young entrepreneurs ages 8-17, covering business ideas by age group, business plan basics, pricing strategies, marketing approaches (neighborhood, social media, school), money management, legal considerations for minors running businesses, scaling strategies, and developing an entrepreneurial mindset. Use when the user asks about young entrepreneur or needs help with related topics. Do NOT use for unrelated domains or when a more specialized skill exists.
license: Apache-2.0
metadata:
  author: foundry-skills
  version: '1.0.0'
  tags: 'teaching entrepreneurship guide'
  category: 'education'
  subcategory: 'teaching'
  depends: ''
  disclaimer: 'none'
  difficulty: 'beginner'
---

# Young Entrepreneur

## When to Use

## Process

1. **Gather requirements.** Ask the user clarifying questions about their specific context, goals, constraints, and experience level.

2. **Analyze the situation.** Review the information provided and identify key factors, challenges, and opportunities relevant to young entrepreneur.

3. **Develop the framework.** Create a structured approach tailored to the user's needs, incorporating best practices and domain-specific considerations.

4. **Deliver actionable output.** Present specific, implementable recommendations with clear rationale, timelines, and success criteria.

5. **Address edge cases.** Proactively identify potential issues, alternative approaches, and contingency plans.

**Use this skill when:**

- User needs guidance on young entrepreneur
- User asks about young entrepreneur best practices or techniques
- User wants a structured approach to young entrepreneur

**Do NOT use this skill when:**

- A more specialized skill exists for the specific subtopic
- The request is outside the scope of young entrepreneur

## Questions to Ask First

1. How old is the young entrepreneur?
2. What are their interests, skills, and hobbies?
3. How much time can they dedicate (school nights, weekends, summer)?
4. Is there any startup money available, or does it need to be zero-cost to start?
5. What level of parent/guardian involvement and supervision is available?
6. Is this for learning/experience, earning money, or both?
7. Are there any neighborhood or community resources available (farmers market, school events)?
8. Does the young person have any prior experience selling or making things?
9. What is the family's comfort level with the young person interacting with customers?
10. Are there school or local regulations about minors conducting business?

---

## Phase 1: Business Ideas by Age Group

### Ages 8-12: Starter Businesses

**Low/No Startup Cost:**
| Business | Startup Cost | Skills Developed |
|----------|-------------|-----------------|
| Lemonade/baked goods stand | $10-$30 | Pricing, customer service, math |
| Pet sitting/dog walking | $0 | Responsibility, scheduling, animal care |
| Yard work (mowing, raking, weeding) | $0 (use family tools) | Physical work, time management |
| Car washing | $10-$20 | Service quality, pricing |
| Handmade cards/bookmarks | $5-$15 | Creativity, marketing |
| Organizing services (garages, closets) | $0 | Problem solving, customer service |
| Plant growing and selling | $10-$20 | Patience, biology, pricing |
| Tutoring younger students | $0 | Teaching, communication |

**Key principles for ages 8-12:**

- Start very small (one product, one service)
- Parent supervision for all customer interactions
- Short time commitment (a few hours per week)
- Focus on learning, not maximizing profit
- Celebrate the experience regardless of financial outcome

### Ages 13-15: Growing Businesses

**Moderate Complexity:**
| Business | Startup Cost | Skills Developed |
|----------|-------------|-----------------|
| Social media content creation | $0-$50 | Digital skills, marketing, consistency |
| Custom t-shirt/sticker design | $50-$200 | Design, e-commerce, branding |
| Reselling (thrift flips, sneakers) | $50-$200 | Market research, negotiation, inventory |
| Photography (events, pets, portraits) | $0-$500 (phone or camera) | Artistic skills, client management |
| Baking/cooking for orders | $30-$100 | Food safety, production, time management |
| Tech support for neighbors | $0 | Technical skills, patience, communication |
| Craft or jewelry making | $30-$100 | Production, quality control, pricing |
| Lawn care with own equipment | $200-$500 | Business operations, route management |
| YouTube channel or podcast | $0-$200 | Content creation, consistency, branding |
| Candle or soap making | $50-$150 | Production, safety, marketing |

### Ages 16-17: Advanced Businesses

**Higher Complexity and Potential:**
| Business | Startup Cost | Skills Developed |
|----------|-------------|-----------------|
| Freelance graphic design | $0-$200 (free tools) | Professional skills, client management |
| Web development | $0-$100 | Technical skills, project management |
| Mobile car detailing | $200-$500 | Business management, quality standards |
| Event DJ | $500-$2,000 | Performance, marketing, booking |
| Tutoring business (multiple subjects) | $0-$50 | Teaching, scheduling, scaling |
| E-commerce store (Etsy, Shopify) | $50-$300 | Digital business, inventory, shipping |
| Social media management | $0 | Digital marketing, client relationships |
| Mobile phone repair | $100-$300 | Technical skills, customer service |
| Personal training or coaching | $0-$200 | Certification, client management |
| App development | $0-$100 | Coding, product development |

---

## Phase 2: Business Plan Basics

### Youth Business Plan Template

```
MY BUSINESS PLAN
=================
Business Name: _________________________________
Owner(s): _________________________________
Date: _________________________________

1. WHAT IS MY BUSINESS?
   What product or service am I selling?
   _________________________________________________
   _________________________________________________

2. WHO IS MY CUSTOMER?
   Who will buy from me? (Be specific)
   _________________________________________________
   _________________________________________________

3. WHAT PROBLEM DO I SOLVE?
   Why does my customer need this?
   _________________________________________________
   _________________________________________________

4. HOW IS MY BUSINESS DIFFERENT?
   What makes mine special compared to what already exists?
   _________________________________________________
   _________________________________________________

5. HOW MUCH WILL IT COST TO START?
   Materials/supplies:    $__________
   Equipment:             $__________
   Marketing:             $__________
   Other:                 $__________
   TOTAL STARTUP COST:    $__________

6. HOW MUCH WILL I CHARGE?
   Price per item/service: $__________
   How I decided this price: ________________________

7. HOW WILL PEOPLE FIND OUT ABOUT MY BUSINESS?
   _________________________________________________
   _________________________________________________

8. HOW MUCH DO I WANT TO EARN?
   Per week: $__________
   Per month: $__________

9. WHAT DO I NEED TO LEARN OR GET BETTER AT?
   _________________________________________________
   _________________________________________________

10. WHO CAN HELP ME?
    _________________________________________________
    _________________________________________________
```

### Advanced Business Plan (Ages 14+)

Add these sections to the basic plan:

**Market Research:**

- Who are your competitors?
- What do they charge?
- What do they do well? Where do they fall short?
- How many potential customers are in your area?

**Financial Projections (Simple):**

```
MONTHLY FINANCIAL PROJECTION
==============================
Revenue (items sold x price):        $__________

Expenses:
  Materials/supplies:                 $__________
  Marketing:                          $__________
  Transportation:                     $__________
  Equipment maintenance:              $__________
  Other:                              $__________
  TOTAL EXPENSES:                     $__________

PROFIT (Revenue - Expenses):          $__________
PROFIT MARGIN (Profit / Revenue x 100): _________%
```

**Growth Plan:**

- What is the next step after the business is running?
- How could you serve more customers?
- What new products or services could you add?
- At what point would you need help?

---

## Phase 3: Pricing Strategy

### How to Set Prices

**Step 1: Calculate Your Costs**

```
Cost per item/service:
  Materials:                $__________
  Your time (hours x rate): $__________
  Other costs:              $__________
  TOTAL COST:               $__________
```

**Step 2: Research the Market**

- What do competitors charge for similar products/services?
- What are customers willing to pay?
- Is this a premium or budget offering?

**Step 3: Set Your Price**

```
Cost + Profit Margin = Price

Example (baked goods):
  Ingredients per batch of 12 cookies: $4.00
  Packaging: $2.00
  Your time (1 hour): priceless learning, but consider it
  Cost per cookie: $0.50
  Desired profit margin: 100%
  Price per cookie: $1.00
  Per batch: $12.00
```

### Pricing Strategies for Young Entrepreneurs

| Strategy             | When to Use                | Example                                     |
| -------------------- | -------------------------- | ------------------------------------------- |
| Cost-plus pricing    | Standard products          | Material cost + desired profit              |
| Competitive pricing  | Similar services exist     | Match or slightly undercut competitors      |
| Value-based pricing  | Unique or premium offering | Price based on what customer values         |
| Bundle pricing       | Multiple items/services    | 3 bracelets for $10 instead of $4 each      |
| Introductory pricing | New business launch        | 20% off first purchase                      |
| Tiered pricing       | Services with options      | Basic car wash $10, Premium $20, Deluxe $30 |

### Common Pricing Mistakes

- Pricing too low (undervaluing your work, unsustainable)
- Not accounting for all costs (materials, transportation, packaging)
- Not adjusting prices as costs change
- Giving too many discounts
- Not having different price points for different customers

---

## Phase 4: Marketing

### Marketing by Channel

**Neighborhood Marketing (All Ages):**

- Door-to-door flyers (with parent permission and supervision)
- Yard signs for service businesses
- Business cards to hand out
- Word of mouth (tell everyone you know)
- Community bulletin boards (library, community center, grocery store)
- Neighborhood Facebook groups or Nextdoor (parent can post)

**School Marketing (With Permission):**

- School bulletin boards
- Morning announcements
- School newspaper or newsletter
- Club and organization partnerships
- School events (fairs, markets, fundraisers)
- Word of mouth among classmates

**Social Media Marketing (Ages 13+, with parental awareness):**

- Instagram (visual products, portfolio showcase)
- TikTok (behind-the-scenes, process videos, tutorials)
- YouTube (longer content, tutorials, reviews)
- Facebook Marketplace (selling items locally)
- Etsy (handmade products, digital downloads)
- Pinterest (visual products, driving traffic to shop)

### Creating Marketing Materials

**Business Card Essentials:**

- Business name
- Your name
- What you offer (one sentence)
- Contact information (parent-supervised email or phone)
- Social media handle (if applicable)

**Flyer Template:**

```
[BUSINESS NAME]
[Eye-catching image]

[What you offer - 1-2 sentences]

[3 benefits for the customer]
- Benefit 1
- Benefit 2
- Benefit 3

[Special offer if applicable]

CONTACT: [Parent-supervised contact information]
[Social media handles]
```

### The 30-Second Elevator Pitch

Practice this with the young entrepreneur:
"Hi, I'm [name] and I run [business name]. I [what you do] for [who you serve]. What makes my [product/service] special is [unique selling point]. Can I tell you more about it?"

---

## Phase 5: Money Management

### Tracking Income and Expenses

```
BUSINESS MONEY TRACKER
========================
Month: _______________

MONEY IN:
Date    | Source              | Amount
--------+---------------------+-----------
_______ | ___________________ | $________
_______ | ___________________ | $________
_______ | ___________________ | $________
TOTAL INCOME:                   $________

MONEY OUT:
Date    | What I Bought       | Amount
--------+---------------------+-----------
_______ | ___________________ | $________
_______ | ___________________ | $________
_______ | ___________________ | $________
TOTAL EXPENSES:                 $________

PROFIT THIS MONTH:              $________
```

### The 3-Jar (or 3-Account) System

Teach young entrepreneurs to divide their profits:

| Jar      | Percentage | Purpose                                                     |
| -------- | ---------- | ----------------------------------------------------------- |
| Save     | 30-40%     | Long-term savings (college, big goal)                       |
| Reinvest | 30-40%     | Put back into the business (supplies, equipment, marketing) |
| Spend    | 20-30%     | Enjoy the fruits of your labor                              |

**For ages 13+, consider adding:**

- A "Give" category (10% to charity or cause they care about)
- A checking account (many banks offer youth accounts)
- A savings account for business reserves

### Understanding Profit vs. Revenue

This is one of the most important lessons for a young entrepreneur:

```
Revenue (total money collected):     $100
- Cost of materials:                 $30
- Other expenses:                    $10
= Profit (what you actually earned): $60

Revenue is NOT how much you made.
Profit is how much you made.
```

---

## Phase 6: Legal Considerations for Minors

### General Rules (Vary by Location)

**What minors generally CAN do:**

- Sell goods at reasonable scale (lemonade stands, crafts, baked goods)
- Provide services (lawn care, pet sitting, tutoring)
- Open savings accounts (with parent/guardian as joint holder)
- Sell online (most platforms require age 18, so parent must create account)
- Accept cash and some digital payments

**What typically requires adult involvement:**

- Signing contracts (minors generally cannot enter binding contracts)
- Opening business bank accounts
- Registering a business (parent/guardian must sign)
- Obtaining business licenses or permits
- Tax filing (required if earning above certain thresholds)
- Online sales platforms (account in parent's name)
- Accepting credit card payments (merchant accounts require age 18)

### Tax Basics for Young Entrepreneurs

- Income from self-employment may be taxable above certain thresholds
- Keep all receipts and records from the start
- Business expenses can offset income
- Parents may need to report the income on their tax return or file separately
- Consult a tax professional if the business earns significant income
- Self-employment tax applies to net earnings above the threshold

### Food Business Considerations

- Check local cottage food laws (many states allow home baking with limitations)
- Food safety certifications may be required
- Allergen labeling requirements
- Commercial kitchen requirements (for higher volume)
- Health department regulations vary by state and municipality

### Insurance Considerations

- Parent's homeowner's or renter's insurance may cover some liability
- For service businesses (lawn care, pet sitting), consider additional coverage
- For events or larger operations, general liability insurance may be advisable
- Discuss with insurance agent as the business grows

---

## Phase 7: Scaling the Business

### Signs You Are Ready to Grow

- More demand than you can handle alone
- Consistent income for 3+ months
- Clear processes that could be taught to someone else
- Available time or resources to take on more
- Customers asking for additional products or services

### Growth Strategies for Young Businesses

| Strategy                         | Example                                   | Complexity  |
| -------------------------------- | ----------------------------------------- | ----------- |
| Increase prices                  | Raise from $8 to $10 per service          | Low         |
| Add complementary products       | Lawn care + leaf removal + snow shoveling | Medium      |
| Expand geographic area           | Serve more neighborhoods                  | Medium      |
| Bring on a partner or helper     | Pay a friend to help with orders          | Medium      |
| Move to online sales             | Open Etsy shop (parent's account)         | Medium      |
| Create packages or subscriptions | Monthly lawn care plan                    | Medium-High |
| Wholesale to local shops         | Sell crafts to a boutique                 | High        |

### When Growing, Maintain Quality

- Do not take on more than you can deliver well
- Customer satisfaction is more important than customer count
- One unhappy customer tells more people than ten happy ones
- It is okay to say "I am fully booked right now"
- Quality and reliability build a reputation that markets itself

---

## Phase 8: Entrepreneurial Mindset

### Key Mindset Lessons

**1. Failure Is Data, Not Defeat**
Every business has setbacks. The lemonade stand with no customers teaches market research. The craft that did not sell teaches product-market fit. Normalize failure as part of the process.

**2. Listen to Your Customers**
Ask customers what they want. Watch what they buy. Their behavior tells you more than your assumptions.

**3. Start Before You Are Ready**
You will never feel 100% prepared. Start small, learn as you go, and improve continuously.

**4. Solve Problems, Not Just Sell Products**
The best businesses solve a real problem for real people. "What frustrates people?" is a better question than "What can I sell?"

**5. Consistency Beats Perfection**
Showing up reliably matters more than being flawless. Customers value dependability.

**6. Ask for Help**
Successful entrepreneurs build networks. Mentors, family, teachers, other business owners -- learning from others accelerates growth.

**7. Manage Your Time**
School comes first. A business should enhance your education and life, not replace it.

### Entrepreneurship and Education Connection

- Math: Pricing, profit calculations, budgeting, percentages
- Language Arts: Marketing copy, business communication, pitching
- Social Studies: Economics, supply and demand, market forces
- Science: Product development, food science, experimentation
- Art: Design, branding, visual marketing
- Technology: Digital tools, social media, website building

### Resources for Young Entrepreneurs

- NFTE (Network for Teaching Entrepreneurship) -- youth programs
- Junior Achievement -- business education programs
- Lemonade Day -- national program for young entrepreneurs
- Local Small Business Development Centers (SBDC) -- free mentoring
- SCORE -- free business mentoring from experienced entrepreneurs
- Library resources on business and entrepreneurship for youth

---

## Parent/Guardian Role

### How to Support Without Taking Over

- Help with safety and legal compliance, but let the young person make business decisions
- Ask guiding questions instead of giving answers
- Let them experience small failures in safe contexts
- Help with transportation and logistics as needed
- Provide honest feedback when asked
- Celebrate effort and learning, not just profits
- Set clear boundaries (homework before business, screen time limits apply)
- Help with financial record-keeping until they can do it themselves
- Be the "silent partner" -- available but not running the show

The goal is not to build a successful business (though that is great). The goal is to build a capable, confident, financially literate young person who understands how value is created and exchanged in the world.

## Output Format

Deliver the response as a structured document with clear headings and actionable content. Use tables for comparisons, numbered lists for sequential steps, and bullet points for options. Include specific examples where applicable.

```
[Young Entrepreneur deliverable]
1. Context and objectives
2. Analysis or framework
3. Specific recommendations with rationale
4. Action items with timeline
```

## Example

**Input:** "Help me with young entrepreneur for a mid-size project."

**Output:** A complete young entrepreneur framework tailored to the specific context, with actionable steps, relevant considerations, and measurable outcomes.

## Edge Cases

- **Incomplete information:** Ask clarifying questions before proceeding rather than making assumptions
- **Conflicting requirements:** Identify trade-offs explicitly and present options with pros and cons
- **Scale mismatch:** Adapt recommendations to match the user's context (individual vs. team vs. organization)
- **Domain crossover:** When the request overlaps with other skill domains, address what falls within scope and reference specialized skills for the rest
