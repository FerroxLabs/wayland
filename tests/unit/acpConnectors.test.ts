...
describe('createGenericSpawnConfig - Windows path handling', () => {
  ...
  it('parses a quoted Unix path with spaces into a bare command, no shell', () => {
    ...
    const config = createGenericSpawnConfig(
      '"/Users/x/Library/Application Support/Wayland/engine-overrides/linux-arm64/engine"',
      '/cwd',
      ['acp-host'],
      undefined,
      { PATH: '/usr/bin' }
    ...
    expect(config.command).toBe('/Users/x/Library/Application Support/Wayland/engine-overrides/linux-arm64/engine');
