...
describe('FluxRouterHero', () => {
  ...
  it('renders the calm confirmation (no fabricated numbers) when metrics are null', async () => {
    ...
    // The "86 models" count is the only number, and it comes from the locked
    // product figure — not from any daemon stat.
    expect(screen.getByTestId('flux-router-confirmation').textContent).toContain('count=86');
  });
