...
try {
  ...
  const manifest = {
    ...
    revision: 'cross-worktree-test-1',
    gates: { REQUIRED: { all: ['TEST'], any: [] } },
  }
  ...
  assert.equal((await checkGate(args)).ok, true, 'an externally shared accepted receipt must satisfy a clean successor worktree')
