  ...
  return (
    <div
      className={classNames(
        'rounded-12px bg-[var(--bg-2)] border border-[var(--bg-3)] overflow-hidden',
        className
      )}
    >
      {hasHeader && (
        <div className='flex items-center gap-8px px-16px py-12px border-b border-[var(--bg-3)]'>
          {Icon && <Icon size={16} className='text-[var(--text-muted)] shrink-0' />}
          {title && <span className='text-14px font-medium text-[var(--text-primary)] flex-1'>{title}</span>}
