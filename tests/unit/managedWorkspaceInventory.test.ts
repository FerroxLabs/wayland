...
describe('collectManagedWorkspaceInventory', () => {
  ...
  it('reports an old empty direct child as quarantine-eligible without mutating it', async () => {
    const candidate = await makeCandidate();
    const report = await collect();

    expect(report).toMatchObject({
      complete: true,
      summary: { discovered: 1, preserved: 0, quarantineEligible: 1, unknown: 0 },
    });
    expect(report.entries[0]).toMatchObject({
      canonicalPath: await fs.realpath(candidate),
      decision: { disposition: 'quarantine-eligible', classifications: ['empty-abandoned'] },
    });
  ...
  it('canonicalizes aliases and preserves every referenced authority', async () => {
    ...
    expect(entry.decision.classifications).toEqual(['referenced', 'scheduled', 'artifact-bearing', 'user-promoted']);
  ...
  it('captures a canonical directory from Wayland’s CLI-safe work-root alias', async () => {
    ...
    try {
      ...
      expect(report).toMatchObject({
        ...
        complete: true,
        summary: { discovered: 1, quarantineEligible: 1 },
      });
