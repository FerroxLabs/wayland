...
describe('buildAgentConversationParams - account binding stickiness', () => {
  ...
  it('lets two conversations on the same backend hold different accounts', () => {
    const a = buildAgentConversationParams({ backend: 'fuigo', name: 'A', workspace: '/tmp/a', model: baseModel('acct_alpha') });
    const b = buildAgentConversationParams({ backend: 'fuigo', name: 'B', workspace: '/tmp/b', model: baseModel('acct_beta') });
  ...
  it('a binding with no accountId resolves to the default account', () => {
    const params = buildAgentConversationParams({ backend: 'fuigo', name: 'A', workspace: '/tmp/a', model: baseModel() });
