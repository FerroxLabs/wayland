...
- At least 2GB RAM
- Wayland `.deb` package from [Releases](https://github.com/TradeCanyon/Wayland/releases)
...
wget https://github.com/TradeCanyon/Wayland/releases/latest/download/Wayland-linux-amd64.deb
...
在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 Wayland WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- Wayland `.deb` 安装包（[下载地址](https://github.com/TradeCanyon/Wayland/releases)）
...
Wayland 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/Wayland/start-wayland.sh`：

```bash
#!/bin/bash
# Wayland WebUI 无头启动脚本
# 用法: ./start-wayland.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Wayland 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 Wayland WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Wayland 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "Wayland 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Wayland 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "Wayland 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
Wayland WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（Wayland Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/Wayland/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/Wayland/proxy.pac"`
...
| Wayland 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
