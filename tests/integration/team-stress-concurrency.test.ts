...
class InMemoryTeamRepository implements ITeamRepository {
  ...
  async findStaleLeasedTasks(now: number): Promise<TeamTask[]> {
    return [...this.tasks.values()]
      .filter((t) => t.status === 'in_progress' && t.leaseExpiresAt !== undefined && t.leaseExpiresAt < now)
      .map((t) => structuredClone(t));
  }
  ...
  async renewLease(id: string, owner: string, leaseExpiresAt: number, now: number): Promise<boolean> {
    ...
    this.tasks.set(id, { ...t, leaseOwner: owner, leaseExpiresAt, lastHeartbeat: now, updatedAt: now });
    return true;
  }
  ...
  async markZombie(id: string, now: number): Promise<boolean> {
    ...
    const lapsed = t.leaseExpiresAt !== undefined && t.leaseExpiresAt < now;
    const legacyStale = t.leaseExpiresAt === undefined && t.updatedAt < now - LEASE_TTL_MS;
    if (!lapsed && !legacyStale) return false;
    this.tasks.set(id, { ...t, status: 'zombie', updatedAt: now });
    return true;
  }
  ...
  async recoverVerifyingTask(id: string, metadataPatch: Record<string, unknown>, now: number): Promise<boolean> {
    ...
    this.tasks.set(id, { ...t, status: 'completed', metadata: { ...t.metadata, ...metadataPatch }, updatedAt: now });
    return true;
  }

  async findZombieTasks(): Promise<TeamTask[]> {
    return [...this.tasks.values()].filter((t) => t.status === 'zombie').map((t) => structuredClone(t));
  }

  async reclaimZombie(id: string, now: number): Promise<'requeued' | 'exhausted' | 'skipped'> {
    const t = this.tasks.get(id);
    if (!t || t.status !== 'zombie') return 'skipped';
  ...
  async findStaleVerifyingTasks(now: number, staleMs: number): Promise<TeamTask[]> {
    return [...this.tasks.values()]
      .filter((t) => t.status === 'verifying' && t.updatedAt < now - staleMs)
      .map((t) => structuredClone(t));
  }
