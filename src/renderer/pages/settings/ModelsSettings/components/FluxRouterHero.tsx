...
import { Lightning, Link as LinkIcon, Loading, Right } from '@icon-park/react';
...
const FluxRouterHero: React.FC<FluxRouterHeroProps> = ({ connected, onConnectKey }) => {
  ...
  return (
    <div className={styles.hero} data-testid='flux-router-hero' data-state='disconnected'>
      <div className={styles.recommendedTag}>{t('settings.modelsPage.flux.recommended')}</div>
      ...
      {showConnect ? (
        ...
        <div className={styles.ctaRow}>
          <Button type='primary' icon={<Lightning theme='filled' />} onClick={() => setShowConnect(true)}>
