...
/**
 * ACP available command type - subset of SlashCommandItem for the ACP protocol layer
 */
...
export class AcpAgent {
  ...
  constructor(config: AcpAgentConfig) {
    ...
    this.connection = new AcpConnection(config.waylandNanoActivation ?? null);
  ...
  /**
   * Check whether a tool is a chrome-devtools navigation tool.
   *
  ...
  /**
   * Extract URL from a navigation tool's permission request data.
   *
   * Delegates to NavigationInterceptor for unified logic.
   */
  ...
  /**
   * Handle an intercepted navigation tool by emitting a preview_open event.
   */
  ...
  // Start the ACP connection and session
  ...
  async sendMessage(data: { content: string; files?: string[]; msg_id?: string }): Promise<AcpResult> {
    ...
    try {
      ...
      // Add @ prefix to all uploaded files (including images) using the full path so Claude CLI can read them
      if (data.files && data.files.length > 0) {
        const fileRefs = data.files
          .map((filePath) => {
            ...
            // Wrap paths containing spaces in quotes so Claude CLI parses them correctly
  ...
  private handleSessionUpdate(data: AcpSessionUpdate): void {
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        // All messages are emitted directly without complex replacement logic
  ...
  private handlePermissionRequest(data: AcpPermissionRequest): Promise<{ optionId: string }> {
    return new Promise((resolve, reject) => {
      ...
      // (the agent needs chrome-devtools to fetch page content)
      ...
      if (this.isNavigationTool(toolName)) {
        ...
        // Track for later extraction from result if URL is not available now
        this.pendingNavigationTools.add(requestId);
      }

      // Check for a duplicate permission request
      if (this.pendingPermissions.has(requestId)) {
        // If it is a duplicate, clean up the old one first
      ...
      // Ensure the permission message is always emitted, even if there are async issues
      try {
  ...
  private handleEndTurn(turnId?: string): void {
    ...
    // Use the signal callback to send the end_turn event without adding it to the message list
    if (this.onSignalEvent) {
      this.onSignalEvent({
        type: 'finish',
        conversation_id: this.id,
  ...
  private emitPermissionRequest(data: AcpPermissionRequest): void {
    // Important: register the toolCall from the permission request into the adapter's activeToolCalls
    // so that subsequent tool_call_update events can locate the corresponding tool call.
    if (data.toolCall) {
      // Map the kind from the permission request to a valid type
      ...
      // Create a tool call message to register it in activeToolCalls
      this.adapter.convertSessionUpdate(toolCallUpdate);
    }

    // Use onSignalEvent instead of emitMessage so the message is not persisted to the database.
    // A permission request is a transient interaction message that becomes meaningless once the user responds.
    if (this.onSignalEvent) {
      this.onSignalEvent({
        type: 'acp_permission',
        conversation_id: this.id,
  ...
  /**
   * Create a new session or resume an existing one, and notify the upper layer if the session ID changed.
   *
  ...
  private async ensureBackendAuth(backend: AcpBackend, loginArg: string): Promise<void> {
    try {
      ...
      // Use the same command-resolution logic as AcpConnection
  ...
  private async performAuthentication(): Promise<void> {
    try {
      ...
      // Attempt to create/resume session directly to check if already authenticated
      try {
        await this.createOrResumeSession();
        this.emitStatusMessage('authenticated');
        return;
      } catch (_err) {
        // Authentication required - proceed with conditional warmup attempt
      }

      // Conditional warmup: only invoke the backend CLI login to refresh the token when auth is needed
      ...
      try {
        ...
        // If still failing, guide the user to log in manually
        this.emitStatusMessage('error');
      }
    } catch (error) {
      this.emitStatusMessage('error');
