...
description: |
  Writes email newsletters with subject line options, section structure, calls-to-
...
- The user wants a LinkedIn article or long-form published essay -- use `linkedin-article` instead
- The user wants an automated drip sequence or onboarding email series -- use `email-sequence-writing` instead
- The user needs to write the full text of a product launch announcement sent by email -- use `product-launch-email` instead
- The user wants to write an internal all-hands or company-wide update memo -- use `internal-communications` instead
...
- [ ] Subject line is under 50 characters and passes a basic spam trigger check (no all-caps words, no excessive punctuation, no "free" or "guaranteed")
- [ ] Preview text is 85-100 characters and does not duplicate the subject line
- [ ] The newsletter opens with something other than a greeting, welcome, or day-of-week phrase
- [ ] Total word count is within the chosen format's target range (400-800 words for most formats)
- [ ] No paragraph exceeds 3 sentences
- [ ] There is exactly one primary CTA
- [ ] All link text is descriptive (no "click here" or "learn more" without context)
- [ ] No content requires images to be understood (images may be blocked by email clients)
- [ ] The entire email reads coherently in plain text (no formatting that depends on HTML rendering)
- [ ] The P.S. is present and adds genuine value or personality
