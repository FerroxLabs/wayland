...
description: |
  Produces one section of a sales playbook with talk tracks, common
...
- The user needs a complete sales pitch deck with narrative arc, visual direction, and slide-by-slide content -- use `sales-pitch-deck` instead
...
5. **Landmine questions must work without naming the competitor.** "How does [Competitor] handle cross-project reporting?" is a direct attack and puts prospects on the defensive. "How do you currently pull visibility across all active campaigns when your CMO asks for a status update?" is a landmine -- it surfaces the gap and the prospect concludes the limitation themselves, which is 10x more persuasive.
...
7. **The quick-reference card must be usable during a live call without scrolling.** If it requires more than 30 seconds to scan, it will not be used. Maximum 15 rows. If the content does not fit, trim the playbook section, not the card -- the card is the most-used artifact.

8. **Escalation triggers must be observable conditions, not judgment calls.** "Deal seems complex" is not an escalation trigger. "Prospect mentions a multi-system integration requirement that includes SAP, Workday, or a proprietary internal tool" is an escalation trigger. Reps should not have to guess when to escalate.

9. **Every play must have a disqualifying signal.** A play without disqualifiers gets applied everywhere, which dilutes results and wastes rep time. If the user does not provide disqualifying signals, derive them from the play type -- renewal plays do not apply to accounts with <3 months of usage; displacement plays are low-yield when the prospect has >18 months left on a competitor contract.

10. **Every play must include play health metrics and a maintenance owner.** A playbook that is never reviewed becomes a liability -- competitive data goes stale, proof points leave, and messaging drifts from market reality. Set a win rate threshold for triggering review, assign an owner, and build in a 6-month staleness check as a minimum standard.
