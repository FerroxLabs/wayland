...
_replaces_ the workflow block rather than merging, so the "one line" would have been three keys.
...
The safe design is to emit only on a turn-gated _successful_ end of turn. OpenClaw already carries
...
4. **#910 naming** — done. **#910 functional** — badge/list now agree; whether a scheduled chat
   _belongs_ in Recents is still a product question.
