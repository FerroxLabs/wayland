 ...
 *
 * The retired Core engine's file reader took ABSOLUTE paths only. Handed
 * `.wayland/skills/<name>/SKILL.md` it answered
 ...
 * that engine: Smart Trader was told "that skill is a FILE IN YOUR
 * WORKSPACE at `.wayland/skills/tvcontrol-setup/SKILL.md`. Read that file",
 ...
 *
 * The SHELL is the exception and stays as it is: `cd .wayland/skills/<x>`
 ...
 * `.wayland/skills/` prefix in general.
 */
...
describe('bundled personas load skills the way the engine supports', () => {
  ...
  it.each(personaFiles().map((f) => [path.relative(ASSISTANT_DIR, f), f] as const))(
    ...
    (_name, file) => {
      ...
      // A relative `.wayland/skills/<x>/SKILL.md` reference - i.e. one not
      // preceded by a path root - is the exact string the reader refuses.
      const offenders = [...body.matchAll(/(.{0,60})\.wayland\/skills\/[^\s`'"]*SKILL\.md/g)]
        .filter((m) => !/[/~]$/.test(m[1]))
        .map((m) => m[0].trim());
