...
/**
 * Adapter class to convert ACP messages to Wayland message format
 */
export class AcpAdapter {
  ...
  /**
   * Convert ACP session update to Wayland messages
   */
  ...
  /**
   * Convert ACP session update chunk to Wayland message
   */
  ...
  /**
   * Convert ACP thought chunk to Wayland message
   */
  ...
  private createOrUpdateAcpToolCall(update: ToolCallUpdate): IMessageAcpToolCall | null {
    ...
    // Use toolCallId as msg_id so messages for the same tool call can be merged
    const baseMessage = {
      id: uuid(),
  ...
  private updateAcpToolCall(update: ToolCallUpdateStatus): IMessageAcpToolCall | null {
    ...
    const updatedMessage: IMessageAcpToolCall = {
      ...existingMessage,
      msg_id: toolCallId, // Ensure msg_id is consistent so composeMessage will merge the messages
  ...
  /**
   * Convert plan update to Wayland message
   */
