...
const DEFS = [
  {
    id: 'com.digitalocean/digitalocean-mcp',
    title: 'DigitalOcean',
    tier: 'builder',
    cats: ['infrastructure', 'devops'],
    tags: ['digitalocean', 'cloud', 'hosting'],
    brand: '#0080FF',
    rank: 200,
    desc: "Manage App Platform, droplets, managed databases, Kubernetes, and the container registry - DigitalOcean's official MCP.",
    web: 'https://docs.digitalocean.com/reference/mcp',
    ...
    auth: {
      ...
      providerSignupUrl: 'https://cloud.digitalocean.com/account/api/tokens',
    },
    steps: [
      {
        id: 'install',
  ...
  {
    id: 'com.upstash/upstash-mcp',
    title: 'Upstash',
    tier: 'builder',
    cats: ['database', 'infrastructure'],
    tags: ['upstash', 'redis', 'qstash'],
    brand: '#00E9A3',
    rank: 201,
    desc: "Manage Upstash Redis, QStash, and Workflow resources in natural language - Upstash's official MCP.",
    web: 'https://github.com/upstash/mcp-server',
    ...
    pkg: {
      ...
      env: [
        { name: 'UPSTASH_EMAIL', description: 'Upstash account email.', isRequired: true, isSecret: false },
  ...
  },
  {
    ...
    web: 'https://github.com/twilio-labs/mcp',
    repo: 'https://github.com/twilio-labs/mcp',
    pkg: {
      registryType: 'npm',
      identifier: '@twilio-alpha/mcp',
      version: '0.7.0',
      runtimeHint: 'npx',
      ...
      env: [
        {
          ...
          description: 'AccountSID/APIKeySID:APIKeySecret',
          isRequired: true,
          isSecret: true,
        },
      ],
  ...
  {
    id: 'com.xero/xero-mcp',
    title: 'Xero',
    tier: 'worker',
    cats: ['payments', 'productivity'],
    tags: ['xero', 'accounting', 'invoices'],
    brand: '#13B5EA',
    rank: 203,
    ...
    web: 'https://github.com/XeroAPI/xero-mcp-server',
    ...
    pkg: {
      ...
      runtimeHint: 'npx',
      env: [
        { name: 'XERO_CLIENT_ID', description: 'Xero Custom Connection client ID.', isRequired: true, isSecret: false },
        {
    ...
    auth: {
      method: 'oauth2-byo',
      providerName: 'Xero',
      providerSignupUrl: 'https://developer.xero.com/app/manage',
...
for (const d of DEFS) {
  ...
  const pkg = {
    registryType: d.pkg.registryType,
    identifier: d.pkg.identifier,
    version: d.pkg.version,
    runtimeHint: d.pkg.runtimeHint,
  ...
  const entry = {
    ...
    version: d.pkg.version,
    websiteUrl: d.web,
    repository: { url: d.repo, source: 'github' },
    packages: [pkg],
    remotes: [],
    'x-wayland': {
      tier: d.tier,
  ...
  for (const s of d.steps) {
    ...
    if (s.externalAction)
      y.push(
        `    externalAction: { label: ${JSON.stringify(s.externalAction.label)}, url: ${JSON.stringify(s.externalAction.url)} }`
      );
    if (s.inputs) {
      y.push('    inputs:');
      for (const inp of s.inputs)
        y.push(
          `      - { name: ${inp.name}, label: ${JSON.stringify(inp.label)}${inp.secret ? ', secret: true' : ''} }`
        );
    }
    if (s.warning) {
      y.push('    warning: |');
      for (const l of s.warning.split('\n')) y.push(`      ${l}`);
    }
    if (s.primaryAction)
      y.push(
        `    primaryAction: { label: ${JSON.stringify(s.primaryAction.label)}, action: ${JSON.stringify(s.primaryAction.action)} }`
      );
    y.push('    body: |');
...
console.log(
  `Added ${added}. Catalog now ${catalog.entries.length}. Icon slugs:`,
  DEFS.map((d) => slug(d.id)).join(' ')
);
