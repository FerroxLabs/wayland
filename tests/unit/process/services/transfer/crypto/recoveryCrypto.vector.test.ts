...
// These are correctness vectors for the production 256 MiB Argon2 profile,
// not latency assertions. Concurrent aggregate workers can legitimately push
// the derivation beyond Vitest's former 30 s ceiling on developer machines.
const RECOVERY_VECTOR_TIMEOUT_MS = 90_000;

describe('WT-R1 deterministic vector', () => {
  it(
    'derives the fixed Argon2id v0x13 key',
    async () => {
      ...
      expect(Buffer.from(key).toString('hex')).toBe('aad608b5866cef907f47d5cae529ed01a91301c92c5d5fef46e1a65e394e5742');
    },
    RECOVERY_VECTOR_TIMEOUT_MS
  );

  it(
    'decrypts the fixed XChaCha20-Poly1305 envelope',
    async () => {
      const decrypted = await new RecoveryCryptoSession().decryptChunk(VECTOR, 'correct horse battery staple');
