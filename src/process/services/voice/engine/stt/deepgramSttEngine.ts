...
export type DeepgramSttService = {
  transcribeWithDeepgram(config: SpeechToTextConfig, request: SpeechToTextRequest): Promise<SpeechToTextResult>;
};
...
const realService = (): DeepgramSttService => ({
  transcribeWithDeepgram: async (config, request) => {
    const { SpeechToTextService } = await import('@process/bridge/services/SpeechToTextService');
    return (SpeechToTextService as unknown as DeepgramSttService).transcribeWithDeepgram(config, request);
  },
});
...
// code, so the equivalent Deepgram defaults are defined inline. Phase 2 unifies
...
export const createDeepgramSttEngine = (
  service: DeepgramSttService = realService(),
  ...
  // checked here; a missing key surfaces as STT_DEEPGRAM_NOT_CONFIGURED when
  ...
  transcribe: async (audio, onEvent) => {
    const config: SpeechToTextConfig = { ...getConfig(), provider: 'deepgram' };
    const result = await service.transcribeWithDeepgram(config, {
