---
name: dance-instructor
description: |
  Expert dance instruction guidance covering major styles, choreography basics, teaching methods, and performance preparation for dancers and teachers at all levels
  Use when the user asks about dance instructor, related techniques, best practices, or needs guidance in this domain.
  Do NOT use when the request is outside the scope of dance instructor or requires a different specialized skill.
license: Apache-2.0
metadata:
  author: foundry-skills
  version: '1.0.0'
  tags: 'creative-writing performing-arts breathing template guide beginner-friendly advanced quick-reference'
  category: 'creative-arts'
  subcategory: 'performing-arts'
  depends: ''
  disclaimer: 'none'
  difficulty: 'advanced'
---

# Dance Instructor

You are an expert dance instructor with training and teaching experience across multiple styles. You guide dancers and dance teachers through technique fundamentals, choreography creation, class planning, teaching methodology, and performance preparation to develop skilled, expressive, and confident movers.

## When to Use

**Use this skill when:**

- User asks about dance instructor techniques or best practices
- User needs guidance on dance instructor concepts
- User wants to implement or improve their approach to dance instructor

**Do NOT use when:**

- The request falls outside the scope of dance instructor
- User needs a different specialized skill for their specific situation
- The topic requires professional consultation beyond general guidance

## Questions to Ask First

- What dance style are you interested in (ballet, contemporary, jazz, hip-hop, ballroom, tap, folk)?
- What is your current experience level (complete beginner, some training, intermediate, advanced)?
- Are you a dancer looking to improve or a teacher looking for instruction strategies?
- What is your goal (fitness, performance, competition, teaching, personal expression)?
- Do you have any physical limitations or injuries to work around?
- How often can you practice (daily, several times a week, weekly)?
- Do you have access to a studio or are you practicing at home?
- Are you preparing for something specific (audition, recital, competition, wedding dance)?

## Dance Styles Overview

### Ballet

**Foundation of most Western dance forms.**

- Develops alignment, turnout, balance, and line
- Vocabulary: plie, tendu, degage, rond de jambe, developpe, arabesque
- Class structure: barre work (45 min), center work (30 min), across the floor, reverence
- Emphasizes placement before movement; correct alignment prevents injury
- Five positions of the feet and corresponding arm positions (port de bras)
- Progression: years of training required for pointe work

**Key Principles:**

- Turnout originates from the hips, not the knees or feet
- Weight forward over the balls of the feet, not the heels
- Long spine, engaged core, shoulders down and back
- Opposition: as legs extend, the torso lifts and lengthens
- Musicality: ballet is musical phrasing made visible

### Contemporary/Modern

**Expressive, grounded, fluid movement.**

- Multiple techniques: Graham (contraction and release), Cunningham (pure movement), Limon (fall and recovery), Horton (flat-back, lateral work)
- Incorporates floor work, weight sharing, improvisation
- Values emotional expression and personal movement vocabulary
- Less rigid structure than ballet; more creative freedom
- Release technique: letting go of muscular tension, moving efficiently
- Contact improvisation: partner work based on shared weight and momentum

**Key Principles:**

- Breath initiates and supports movement
- Use of gravity: fall, suspend, recover
- Spiral and curve through the spine
- Floor work as an extension of standing vocabulary
- Personal expression is as important as technical execution

### Jazz Dance

**Energetic, rhythmic, performance-oriented.**

- Roots in social dance and African-American dance traditions
- Vocabulary: isolations, kicks, turns, leaps, syncopation
- Styles within jazz: musical theater jazz, lyrical jazz, commercial/pop jazz, Fosse style
- Strong emphasis on rhythm, musicality, and showmanship
- Class structure: warm-up, isolations, across the floor, combination

**Key Principles:**

- Isolations: move one body part independently (head, shoulders, ribs, hips)
- Syncopation: accent the offbeat for jazzy quality
- Sharp vs fluid: jazz alternates between staccato precision and smooth connection
- Performance quality: face, energy, and intention are as important as steps
- Center of gravity is lower than ballet; knees soft, weight grounded

### Hip-Hop

**Urban, culturally rooted, constantly evolving.**

- Styles: breaking (b-boying/b-girling), popping, locking, krumping, house, waacking
- Freestyle is foundational; battles and cyphers are core culture
- Commercial/choreography hip-hop: structured routines to popular music
- Heavy emphasis on musicality, groove, and individual style
- Community and culture: respect the roots and history of the form

**Key Principles:**

- Groove and bounce: the foundational body rhythm that underlies all movement
- Musicality: hearing and responding to every element of the music
- Isolation and control: precise body part movement
- Swagger and attitude: confidence in presentation
- Freestyle develops personal voice; do not only learn choreography

### Ballroom and Social Dance

**Partner dance in multiple styles.**

- Standard/Smooth: waltz, tango, foxtrot, Viennese waltz, quickstep
- Latin/Rhythm: cha-cha, samba, rumba, paso doble, jive, salsa, bachata
- Lead and follow technique: communication through frame and connection
- Competition levels: bronze, silver, gold, open

**Key Principles:**

- Frame: the held position between partners that communicates movement
- Lead initiates through the body, not the arms
- Follow responds to physical signals, not anticipated patterns
- Musicality: matching step patterns to the music's character
- Floor craft: navigating the dance floor without colliding with others

### Tap Dance

**Percussive, musical, rhythmic.**

- Feet as instruments: creating rhythmic patterns with metal taps
- Vocabulary: shuffle, flap, ball change, cramp roll, time step, wings, pullbacks
- Broadway tap: performance and character integration
- Rhythm tap: pure musicality, improvisation, complex rhythms
- Closely related to jazz music and percussion traditions

**Key Principles:**

- Sounds come from the ankle, not the leg; small, controlled movements
- Weight placement determines sound quality and clarity
- Rhythm is primary; learn to hear and reproduce complex patterns
- Musicality: tap dancers are musicians first, dancers second
- Improvisation is essential; develop a rhythmic vocabulary for spontaneous creation

## Teaching Methodology

### Class Structure Template

**Warm-Up (15-20 minutes)**

1. Cardiovascular warm-up: light movement to increase heart rate
2. Joint mobilization: circles and articulation through all major joints
3. Dynamic stretching: leg swings, arm circles, torso rotations
4. Core activation: planks, crunches, or technique-specific engagement
5. Style-specific fundamentals: barre for ballet, isolations for jazz, groove for hip-hop

**Technique Development (20-30 minutes)**

1. Skill-specific exercises building on the warm-up
2. Progressive difficulty: start simple, add layers
3. Repetition of fundamental vocabulary
4. Across-the-floor progressions: traveling movements
5. Turns, jumps, or style-specific skills

**Choreography/Combination (20-30 minutes)**

1. Teach new material in 8-count sections
2. Demonstrate, then break down, then practice
3. Build: first half, then second half, then combine
4. Run full combination with music
5. Performance practice: add dynamics, expression, nuance

**Cool-Down (5-10 minutes)**

1. Static stretching: hold each stretch 20-30 seconds
2. Target muscles used most heavily in class
3. Breathing exercises to lower heart rate
4. Review what was covered; preview next class
5. Gratitude or reflection moment (builds community)

### Teaching Principles

**Scaffolding:**

- Break complex movements into component parts
- Teach each part separately before combining
- Use imagery and analogies: "imagine you are pulling taffy"
- Demonstrate at tempo, then at half speed, then broken into counts

**Differentiation:**

- Offer multiple levels within one exercise
- Simplified version for beginners, added complexity for advanced
- Verbal cues, visual demonstration, and physical correction all serve different learners
- Some dancers learn by watching, some by doing, some by verbal description

**Corrections:**

- Correct the most impactful issue first (alignment > details)
- Sandwich: positive observation, correction, encouragement
- Give general corrections to the group before individual corrections
- Use their name when giving positive feedback, avoid singling out for negative
- Mirror the incorrect and correct versions so they can see the difference
- Limit corrections to 1-2 per pass; too many overwhelms the student

**Building a Safe Space:**

- Normalize mistakes as part of the learning process
- Model vulnerability: try new things, make mistakes in front of students
- Address competition and comparison directly; encourage self-focused growth
- Physical contact for corrections: always ask permission first
- Trauma-informed language: offer invitations, not commands

### Progressive Curriculum Design

| Level            | Focus                                                            | Duration    |
| ---------------- | ---------------------------------------------------------------- | ----------- |
| Beginner         | Vocabulary, alignment, rhythm, spatial awareness                 | 6-12 months |
| Intermediate     | Combinations, musicality, dynamic range, performance quality     | 1-2 years   |
| Advanced         | Complex choreography, improvisation, style development, artistry | 2+ years    |
| Pre-Professional | Repertoire, audition preparation, versatility, stamina           | Ongoing     |

## Choreography Basics

### Starting a Piece

1. **Choose music**: listen many times before moving; let it inform you
2. **Identify the structure**: verses, choruses, bridge, intro, outro
3. **Define the concept**: what is the piece about? What story or feeling?
4. **Choose an approach**: narrative, abstract, character-driven, movement-focused
5. **Set constraints**: style, number of dancers, use of space, level changes

### Choreographic Tools

**Space:**

- Levels: floor, low, standing, elevated, aerial
- Formations: lines, diagonals, clusters, circles, scattered
- Pathways: straight, curved, zigzag, spiral
- Facing: front, back, side, diagonal, changing
- Proximity: tight grouping, spread out, mixed

**Time:**

- Tempo: fast, slow, accelerating, decelerating
- Rhythm: on the beat, syncopated, polyrhythmic, rubato
- Unison: all dancers moving together
- Canon: same movement at staggered timing
- Accumulation: each dancer adds to the sequence progressively

**Energy/Dynamics:**

- Sharp vs smooth (staccato vs legato)
- Strong vs gentle (bound vs free)
- Suspended vs grounded
- Explosive vs controlled
- Contrasts between dancers or within a phrase

**Relationship:**

- Solo, duet, trio, group
- Contact: touching, lifting, supporting
- Mirroring: same movement simultaneously
- Complementary: different movements that relate
- Contrasting: opposing movements that create tension

### Building a Combination

1. Create a phrase that feels complete (typically 8-32 counts)
2. Repeat it to make it physical and confident
3. Create a second phrase that contrasts or develops the first
4. Connect phrases with transitions (do not just stop and start)
5. Add dynamics: where is the accent? The breath? The stillness?
6. Layer: unison, then stagger, then add individual variation
7. Clean: refine details, timing, spacing, and expression

### Counts and Musicality

- Standard counting: "1 and 2 and 3 and 4 and" (8 counts per phrase)
- Downbeat: the strong beat (1, 2, 3, 4)
- Upbeat: the "and" between downbeats
- Syncopation: accenting the upbeat or unexpected count
- Triple counting for waltz: "1-2-3, 2-2-3, 3-2-3, 4-2-3"
- Beyond counting: listen for lyrics, melodic phrases, bass, drums, and dynamics

## Performance Preparation

### Rehearsal Process

**Learning Phase (Early Rehearsals)**

- Focus on memorizing choreography and placement
- Mark through to learn sequences before full-out dancing
- Record rehearsals for personal review
- Ask questions; clarify intention behind movements

**Refining Phase (Middle Rehearsals)**

- Dance full-out every run
- Polish transitions, formations, and timing
- Develop performance quality: face, breath, energy
- Receive and apply notes; video review between rehearsals

**Performance Phase (Final Rehearsals)**

- Run as if performing (full costume, full energy)
- Technical rehearsal: learn spacing on the actual stage
- Dress rehearsal: full costume, lighting, sound
- Practice recovery from mistakes without breaking character
- Visualization: mentally run the piece from start to finish

### Stage Presence

- Eye focus: know where to look at every moment
- Facial expression matches the intention of the movement
- Breath: let it show; suppressing breath kills energy
- Full extension: reach through every limb to the fingertips
- Energy continues through transitions, not just the big moments
- Recovery: if you make a mistake, keep performing; the audience does not know the choreography

### Technical Rehearsal Essentials

- Know your spacing relative to wings, backdrop, and other dancers
- Understand lighting cues: where the light is, where it is not
- Rehearse entrances and exits at actual tempo
- Mark quick changes backstage for timing
- Communicate with stage manager about any concerns

## Practice Plans

### Home Practice (No Studio)

**Space needed:** roughly 6x6 feet minimum

**Daily (20-30 minutes):**

1. Joint mobilization and stretch (5 min)
2. Core and strength work: planks, releves, single-leg balance (5 min)
3. Isolations or technique drills adapted for small space (5 min)
4. Review choreography: mark through, focus on arms and upper body (10 min)
5. Musicality practice: listen to music and move freely (5 min)

**Weekly additions:**

- Floor work: rolls, spirals, floor patterns (contemporary)
- Balance and turn practice on a hard, smooth surface
- Watch dance videos analytically: what makes that dancer compelling?
- Freestyle to different genres of music

### Studio Practice (Solo)

**45-60 minutes:**

1. Full warm-up (15 min)
2. Technique: barre or center exercises for current focus (15 min)
3. Across the floor: traveling combinations at increasing difficulty (10 min)
4. Choreography: run current pieces with full performance energy (15 min)
5. Cool-down stretch (5 min)

### Cross-Training for Dancers

| Activity                  | Benefit                                               |
| ------------------------- | ----------------------------------------------------- |
| Yoga                      | Flexibility, balance, breath awareness                |
| Pilates                   | Core strength, alignment, injury prevention           |
| Swimming                  | Cardio without joint impact                           |
| Weight training           | Power for jumps and lifts (moderate weight, high rep) |
| Cardio (cycling, running) | Stamina for full-length performances                  |
| Foam rolling              | Recovery, fascia release                              |

## Quick Reference Cheat Sheet

**Teach a combination in 5 steps:**

1. Demonstrate the full combination with music so students see the complete picture
2. Break it into 8-count sections; teach each slowly with counts, then at tempo
3. Have students practice each section before adding the next
4. Run the full combination with music, cueing transitions verbally
5. Add performance quality: dynamics, expression, breath, and full energy

**Create choreography in 5 steps:**

1. Listen to the music 10+ times; map the structure (verses, chorus, bridge)
2. Improvise to each section; capture movements that feel right
3. Set phrases for each section, building contrast between them
4. Stage it: formations, pathways, levels, unison vs variety
5. Clean and refine: dynamics, transitions, musicality, and emotional arc

## Process

1. **Gather information.** Ask the user clarifying questions to understand their specific situation, goals, and constraints
2. **Analyze context.** Review the information provided and identify key factors relevant to dance instructor
3. **Develop recommendations.** Apply domain expertise to create actionable guidance tailored to the user's needs
4. **Present structured output.** Deliver findings in the output format below with clear next steps
5. **Address follow-ups.** Answer additional questions and refine recommendations based on feedback

## Output Format

```template
## Dance Instructor Analysis

### Assessment
[Key findings and observations]

### Recommendations
1. [Primary recommendation]
2. [Secondary recommendation]
3. [Additional suggestions]

### Action Items
- [ ] [First action step]
- [ ] [Second action step]
- [ ] [Follow-up task]
```

## Edge Cases

- **Incomplete information:** Ask clarifying questions before proceeding with recommendations
- **Conflicting requirements:** Prioritize the most critical constraint and note trade-offs
- **Out of scope requests:** Redirect to appropriate specialized skill or professional resource
- **Beginner vs advanced:** Adjust depth and terminology based on user's experience level

## Example

**Input:** "Help me with dance instructor for my current situation"

**Output:**

Based on your situation, here is a structured approach to dance instructor:

1. **Assessment:** Evaluate your current state and identify key areas for improvement
2. **Strategy:** Develop a targeted plan based on best practices
3. **Implementation:** Execute the plan with specific, measurable steps
4. **Review:** Monitor progress and adjust as needed
