...
export type ConstitutionFsExecutor = (input: {
  binaryPath: string;
  binaryFd: number;
  executablePath: '/dev/fd/3' | '/proc/self/fd/3';
  stdin: Buffer;
  timeoutMs: number;
...
function createArchiveKeyInventory(entries: readonly ArchiveAuthenticationKeyWire[]): ConstitutionArchiveAuthenticationKeyInventory {
  if (
    entries.length === 0 ||
    entries.length > 64 ||
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive authentication key inventory is malformed.');
  }
...
export function createTestOnlyConstitutionArchiveAuthenticationKeyInventory(
  ...
  if (process.env.NODE_ENV !== 'test' && process.env.VITEST !== 'true') {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Test-only archive keys are forbidden outside a test process.');
  }
  return createArchiveKeyInventory(entries.map(({ keyId, key }) => ({ keyId, keyBase64: key.toString('base64') })));
}

function archiveKeyWire(inventory: ConstitutionArchiveAuthenticationKeyInventory | undefined): readonly ArchiveAuthenticationKeyWire[] {
  const keys = inventory && inventory[ARCHIVE_KEY_INVENTORY] === true ? archiveKeyMaterial.get(inventory) : undefined;
  if (!keys) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Authenticated archive operation requires OS-unsealed trusted key material.');
  }
  return keys;
}
...
function requestRequiresArchiveKeys(request: Record<string, unknown>): boolean {
  ...
  if (request.operation === 'replace' || request.operation === 'delete') {
    ...
    return !!expected && typeof expected === 'object' && !Array.isArray(expected) && (expected as Record<string, unknown>).present === true;
  }
  if (request.operation === 'reconcile') {
    const facts = request.reconcileFacts;
    return !!facts && typeof facts === 'object' && !Array.isArray(facts) &&
      (typeof (facts as Record<string, unknown>).archiveId === 'string' || typeof (facts as Record<string, unknown>).sourceArchiveId === 'string');
  }
  return false;
}
...
function defaultExecutor(input: Parameters<ConstitutionFsExecutor>[0]): ReturnType<ConstitutionFsExecutor> {
  try {
    ...
    if (failure.code === 'ENOBUFS') {
      throw new ConstitutionFsTransactionError('CONSTITUTION_FS_RESPONSE_OVERSIZE', 'Constitution filesystem helper response exceeded its bound.');
    }
    if (failure.stdout) return { stdout: failure.stdout, status: failure.status ?? 2 };
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_EXEC_FAILED', 'Constitution filesystem helper could not be executed.');
  }
}
...
function assertGuarantees(value: Record<string, unknown>): asserts value is Record<string, unknown> & { guarantees: RequiredGuarantees } {
  assertRecord(value.guarantees);
  if (
    !exactKeys(value.guarantees, ['anchored', 'rootIdentityBound', 'reparseRejected', 'noReplace', 'durable', 'recoveryRetained']) ||
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Helper response did not prove every required transaction guarantee.');
  }
}
...
export function pinConstitutionFsRootAuthority(root: string): ConstitutionFsRootAuthority {
  ...
  try {
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_UNSAFE_ROOT', 'Constitution root cannot be captured at the trusted caller boundary.');
  }
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_UNSAFE_ROOT', 'Constitution root is not a real directory.');
  }
...
function invokeNative(
  ...
  if (!authority || authority.root !== root) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ROOT_IDENTITY_MISMATCH', 'Request root is not bound to the pinned root authority.');
  }
  const current = pinConstitutionFsRootAuthority(root);
  if (current.device !== authority.device || current.inode !== authority.inode) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ROOT_IDENTITY_MISMATCH', 'Constitution root diverged from its initialization-time identity.');
  }
  const authenticatedOperation = ['replace', 'delete', 'restore', 'reconcile', 'pending_inventory'].includes(String(request.operation));
  ...
  if (authenticatedOperation && (!Buffer.isBuffer(journalKey) || journalKey.byteLength !== 32)) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_INVALID_REQUEST', 'Transaction requires a 32-byte OS-unsealed journal authentication key.');
  }
  const wireRequest = {
    ...request,
    ...
    ...(authenticatedArchiveOperation ? { archiveAuthenticationKeys: archiveKeyWire(options.archiveAuthenticationKeys) } : {}),
  };
  const stdin = Buffer.from(JSON.stringify(wireRequest), 'utf8');
  if (stdin.byteLength > MAX_REQUEST_BYTES) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_REQUEST_OVERSIZE', 'Transaction request exceeded its bound.');
  }
  const responseLimit = request.operation === 'read_live'
  ...
  if (result.stdout.byteLength > responseLimit) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_RESPONSE_OVERSIZE', 'Helper response exceeded its bound.');
  }
  let value: unknown;
  try {
  ...
  if (value.ok === false && typeof value.code === 'string') {
    if (!exactKeys(value, ['ok', 'version', 'code', 'message']) || value.version !== 1 || typeof value.message !== 'string') {
      throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Helper error response is malformed.');
    }
...
function restoredContentSha256(payload: ConstitutionFsPayload): `sha256:${string}` {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value) || typeof (value as Record<string, unknown>).content !== 'string') {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_INVALID_REQUEST', 'Restore source archive has no content.');
  }
  return `sha256:${createHash('sha256').update((value as Record<string, string>).content, 'utf8').digest('hex')}`;
}
...
function parseTransactionReceipt(
  ...
  const expectedSha256 = request.operation === 'reconcile' ? request.reconcileFacts.expectedSha256 : request.expected.sha256;
  const expectedPresent = request.operation === 'reconcile' ? request.reconcileFacts.expectedPresent : request.expected.present;
  const effectiveOperation = request.operation === 'reconcile' ? request.reconcileFacts.operation : request.operation;
  const subjectTransactionId = request.operation === 'reconcile' ? request.reconcileTransactionId : request.transactionId;
  const replacementSha256 = request.operation === 'reconcile'
  ...
  const archiveSha256 = request.operation === 'reconcile' ? request.reconcileFacts.archiveSha256 : request.archive?.sha256;
  const sourceArchiveSha256 = request.operation === 'reconcile'
  ...
  const reconcileDisposition = request.operation === 'reconcile' &&
    (value.reconcileDisposition === 'rolled_back' || value.reconcileDisposition === 'rolled_forward')
    ? value.reconcileDisposition
    : null;
  const archiveRealityValid = request.operation === 'reconcile'
    ? reconcileDisposition === 'rolled_forward'
      ...
      : (value.archiveName === null && value.archivedAt === null && value.archiveSha256 === null) ||
        (value.archiveName === `${archiveId}.json` && value.archivedAt === archivedAt && value.archiveSha256 === archiveSha256)
    : value.archiveName === expectedArchiveName &&
      value.archivedAt === (expectedPresent ? (archivedAt ?? null) : null) &&
      value.archiveSha256 === (archiveSha256 ?? null);
  const recoveryRealityValid = request.operation === 'reconcile'
    ? reconcileDisposition === 'rolled_forward'
      ? value.recoveryName === expectedRecoveryName
      : expectedRecoveryName === null ? value.recoveryName === null : value.recoveryName === null || value.recoveryName === expectedRecoveryName
    : value.recoveryName === expectedRecoveryName;
  const expectedFinalSha256 = request.operation === 'reconcile'
    ? reconcileDisposition === 'rolled_forward'
      ? effectiveOperation === 'delete' ? null : (replacementSha256 ?? null)
      : reconcileDisposition === 'rolled_back' ? expectedPreviousSha256 : undefined
    : null;
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Helper response is not exactly bound to the requested transaction.');
  }
...
export function inventoryPendingConstitutionFsTransactions(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Pending inventory response is malformed.');
  }
  return value.pendingTransactions;
}
...
function assertReadOnlyReceipt(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Read-only helper response is not exact.');
  }
}
...
export function readConstitutionFsTarget(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Live-read response is not bound to the requested target.');
  }
  const content = Buffer.from(value.contentBase64, 'base64');
  if (content.toString('base64') !== value.contentBase64) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Live-read content is not canonical base64.');
  }
  const digest = `sha256:${createHash('sha256').update(content).digest('hex')}` as const;
  if (digest !== value.contentSha256) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_DIGEST_MISMATCH', 'Live-read digest does not match its bytes.');
  }
...
function inventoryReadOnly(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Read-only inventory response is malformed.');
  }
...
export function readConstitutionFsArchive(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Archive-read response is malformed.');
  }
  const record = Buffer.from(value.contentBase64, 'base64');
  if (record.toString('base64') !== value.contentBase64) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Archive record is not canonical base64.');
  }
  const sha256 = `sha256:${createHash('sha256').update(record).digest('hex')}` as const;
  if (sha256 !== value.contentSha256) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_DIGEST_MISMATCH', 'Archive record digest does not match its bytes.');
  }
  let parsed: unknown;
  try {
  ...
  if (parsed.archiveId !== archiveId || JSON.stringify(parsed.target) !== JSON.stringify(value.target)) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Archive summary does not match its record.');
  }
...
function sealKeyRequest(root: string, transactionId: string, operation: 'seal_key_inventory' | 'seal_key_read' | 'seal_key_create', keyId?: string, envelope?: Buffer): Record<string, unknown> {
...
function parseSealReceipt(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_GUARANTEE_REJECTED', 'Seal-key response did not prove an exact committed anchored operation.');
  }
  return value;
}
...
export function inventoryConstitutionSealKeys(
  ...
  const value = parseSealReceipt(invokeNative(sealKeyRequest(root, transactionId, 'seal_key_inventory'), binary, options), transactionId, 'seal_key_inventory');
  if (
...
export function readConstitutionSealKey(
  ...
  const value = parseSealReceipt(invokeNative(sealKeyRequest(root, transactionId, 'seal_key_read', keyId), binary, options), transactionId, 'seal_key_read');
  if (value.sealKeyIds !== null || value.sealKeyName !== `${keyId}.json` || typeof value.envelopeBase64 !== 'string' || typeof value.envelopeSha256 !== 'string' || !DIGEST_PATTERN.test(value.envelopeSha256)) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Seal-key read response is malformed.');
  }
  const envelope = Buffer.from(value.envelopeBase64, 'base64');
  if (envelope.toString('base64') !== value.envelopeBase64) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Seal-key read response is not canonical base64.');
  }
  const digest = `sha256:${createHash('sha256').update(envelope).digest('hex')}` as const;
  if (digest !== value.envelopeSha256) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_DIGEST_MISMATCH', 'Seal-key envelope digest does not match its bytes.');
  }
...
export function createConstitutionSealKey(
  ...
  const value = parseSealReceipt(invokeNative(sealKeyRequest(root, transactionId, 'seal_key_create', keyId, envelope), binary, options), transactionId, 'seal_key_create');
  const digest = `sha256:${createHash('sha256').update(envelope).digest('hex')}`;
  if (value.sealKeyIds !== null || value.sealKeyName !== `${keyId}.json` || value.envelopeSha256 !== digest || value.envelopeBase64 !== null) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_MALFORMED_RESPONSE', 'Seal-key create response names or binds the wrong key.');
  }
}
...
function unsealArchiveAuthenticationKey(envelope: Buffer, secretBackend: ConstitutionArchiveSecretBackend): Buffer {
  ...
  try {
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive key envelope is not JSON.');
  }
  assertRecord(parsed);
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive key envelope schema is invalid.');
  }
  ...
  if (parsed.cipher === 'electron-safe-storage') {
    ...
    if (ciphertext.byteLength === 0 || ciphertext.toString('base64') !== parsed.ciphertext) {
      throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive key envelope ciphertext is not canonical.');
    }
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive key envelope cipher is not trusted.');
  }
  let plaintext: string;
  try {
    plaintext = secretBackend.decryptString(encrypted);
  } catch {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Archive key could not be unsealed by the configured secret backend.');
  }
  const key = Buffer.from(plaintext, 'base64');
  if (key.byteLength !== 32 || key.toString('base64') !== plaintext) {
    key.fill(0);
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Unsealed archive key material is malformed.');
  }
  return key;
}
...
export function loadConstitutionArchiveAuthenticationKeys(
  ...
  if (keyIds.length === 0) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'No sealed archive authentication keys exist.');
  }
...
export function createAndSealConstitutionArchiveAuthenticationKey(
  ...
  if (existing.length >= 64) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_LIMIT', 'Archive key history reached its retained-key cap; rotation is blocked until an explicit migration retires old history.');
  }
  ...
  if (encrypted.startsWith(CIPHER_PREFIX)) {
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Secret backend returned an unsupported archive key envelope.');
  }
  createConstitutionSealKey(root, randomUUID(), keyId, Buffer.from(`${JSON.stringify(envelope)}\n`, 'utf8'), binary, options);
  return keyId;
}
...
export function createAuthenticatedConstitutionArchive(
  ...
  if (
    ...
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_INVALID_REQUEST', 'Authenticated archive metadata is invalid.');
  }
  const entry = archiveKeyWire(inventory).find((candidate) => candidate.keyId === input.keyId);
  if (!entry) {
    throw new ConstitutionFsTransactionError('CONSTITUTION_FS_ARCHIVE_KEY_UNAVAILABLE', 'Selected archive key is not in the trusted inventory.');
  }
  const unsigned = {
