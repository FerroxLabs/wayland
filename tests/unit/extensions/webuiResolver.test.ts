...
function createTempExtension(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `wayland-ext-${name}-`));
  tempRoots.push(root);
...
describe('extensions/WebuiResolver', () => {
  ...
  it('should ignore the extension webui contributions when there are no valid apiRoutes/staticAssets', () => {
