...
describe('useAvailableBackends', () => {
  it('exposes available list (detected ∪ fuigo) and a recommend() function', () => {
    ...
    expect(result.current.available).toContain('fuigo');
    ...
    expect(result.current.recommend('codex')).toBe('fuigo');
    expect(result.current.recommend(undefined)).toBe('fuigo');
  });
});
