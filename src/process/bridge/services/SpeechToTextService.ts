...
import { buildSttTurnReceipt } from '@process/services/voice/voiceReceiptFactory';
 ...
 */
const transcribeWithFluxVoice = async (
...
/**
 * VOC-04 speech adapter contract. Each provider is registered against this
 * contract rather than dispatched by a nested ternary. `onDevice` feeds the
 * VoiceReceipt cost derivation (whisper-local runs on-device; the rest are
 * hosted and POST audio off-device).
 */
export interface SpeechToTextAdapter extends VoiceAdapter<SpeechToTextProvider> {
  readonly onDevice: boolean;
  transcribe(config: SpeechToTextConfig, request: SpeechToTextRequest): Promise<SpeechToTextResult>;
}

export const speechToTextRegistry = new VoiceAdapterRegistry<SpeechToTextProvider, SpeechToTextAdapter>();

speechToTextRegistry
  .register({ provider: 'flux-voice', onDevice: false, transcribe: transcribeWithFluxVoice })
  .register({ provider: 'openai', onDevice: false, transcribe: transcribeWithOpenAI })
  .register({ provider: 'deepgram', onDevice: false, transcribe: transcribeWithDeepgram })
  .register({ provider: 'whisper-local', onDevice: true, transcribe: transcribeWithWhisperLocal });

export type SpeechToTextTurn = {
...
export class SpeechToTextService {
  /**
   * Preserved entry point for the IPC bridge — returns just the transcript.
   * The receipt is emitted by the underlying `transcribeTurn`.
   */
  static async transcribe(request: SpeechToTextRequest): Promise<SpeechToTextResult> {
    return (await this.transcribeTurn(request)).result;
  }

  /**
   * Canonical speech-to-text turn. Resolves the provider adapter through the
   * registry, enforces the VOC-03 hosted-consent gate before any hosted call,
   * measures the observed boundary, and returns the transcript together with an
   * authoritative VoiceReceipt — a turn cannot complete without emitting one.
   */
  static async transcribeTurn(request: SpeechToTextRequest): Promise<SpeechToTextTurn> {
    ...
    try {
      ...
      const adapter = speechToTextRegistry.resolve(config.provider);
      const result = await adapter.transcribe(config, request);
      ...
      const receipt = buildSttTurnReceipt({
        ...
        audioBytes: normalizeAudioBuffer(request.audioBuffer),
