...
description: |
  Builds structured stretching routines using static, dynamic, and PNF stretching methods with specific hold times, frequency guidelines, and progression protocols for improving flexibility. Produces a targeted stretching program organized by muscle group and stretching type.
...
- The user wants a yoga sequence with breath-led flow, held poses, and spiritual or meditative framing -- use `yoga-routine-builder` instead
...
- The user asks about foam rolling, lacrosse ball work, or instrument-assisted soft tissue mobilization (IASTM) -- use `foam-rolling-myofascial-release` for those protocols
- The user asks about sport-specific warm-up sequencing for competitive athletic performance (e.g., pre-game NFL wide receiver protocol) -- use `sports-specific-conditioning`
...
- Supine foam roller thoracic extension with chest opening: lying perpendicular across a foam roller at mid-thoracic level, let the arms fall open. This is a passive chest stretch that also addresses thoracic mobility (note: if foam rolling is the primary modality, defer to `foam-rolling-myofascial-release`; the chest-opening component is the stretch here).
...
1. Dynamic warm-up movements (2-3 minutes) -- always first, even for post-workout static sessions if the body has cooled significantly
2. Larger muscle groups first (hips, hamstrings, thoracic spine) -- they generate the most systemic warmth
3. Smaller or more distal muscles second (calves, shoulders, wrists)
4. Deepest holds and PNF last -- save the most demanding neurological work for when the tissue is warmest
5. Relaxation-oriented stretches as the final 2-3 minutes (child's pose, supine twist, knees-to-chest) to down-regulate the nervous system
...
8. **Always provide at least one regression alternative for each stretch.** Users who cannot achieve the prescribed position will either force the wrong position (shifting the stretch to a non-target structure) or skip the stretch entirely. A strap-assisted supine hamstring stretch serves users who cannot maintain a standing forward fold with a flat back. A seated figure-four serves users who cannot access pigeon pose.

9. **Progress hold time before progressing range of motion.** The sequencing error users make is always chasing greater depth before they can sustain quality at their current range. The rule: before increasing the angle or depth of a stretch, the user should be able to hold the current position for 45-60 seconds with intensity in the 4-6/10 range throughout the full hold (not just at the beginning). If the sensation increases to 7+/10 at the 20-second mark, the range is too deep.

10. **If the user has not improved after 6-8 weeks of consistent stretching, do not simply prescribe more volume.** Reassess the flexibility limiter (Step 2 of Process). Common causes of non-response: incorrect technique (lumbar rounding during hamstring stretches steals the stretch from the target muscle), neural tension being mistaken for myofascial tightness, hip joint capsule restriction that does not respond to muscle stretching, or the user is not actually performing stretches with sufficient frequency or intensity. Assign the appropriate assessment before adding sessions.
...
| 7   | Wall calf stretch (straight)     | Gastrocnemius                | Static | 30s                           | 2    | Per leg  | Rear heel flat on floor, lean into wall, knee fully straight, feel the stretch in the upper calf belly                                                                                   | Step calf stretch (heel drop off step edge)          |
| 8   | Wall calf stretch (bent knee)    | Soleus                       | Static | 30s                           | 2    | Per leg  | Same position but 20-degree bend in rear knee -- distinct stretch sensation lower in the leg                                                                                             | Seated foot dorsiflexion stretch                     |
