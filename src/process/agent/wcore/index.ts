...
export class WCoreAgent {
        ...
        if (this.options.managedTempDir) {
          try {
            ...
            this.tvControlScratchError = new Error(
              `TVControl 2.5.3 could not be prepared for this session: ${error instanceof Error ? error.message : String(error)}. Reopen the conversation after checking the bundled connector.`
            );
  ...
  private async ensureTvControlScratchReady(): Promise<void> {
    ...
    if (!this.tvControlScratchReady) {
      throw new Error(
        'TVControl 2.5.3 is waiting for Core to report its writable scratch directory. Reopen the conversation; no collector command was sent.'
      );
    }
  }
