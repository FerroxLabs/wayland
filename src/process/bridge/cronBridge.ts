...
import type { ICronBridgeFailure } from '@/common/adapter/ipcBridge';
import { cronService } from '@process/services/cron/cronServiceSingleton';
import { asCronWorkspaceError } from './cronWorkspaceError';
...
/**
 * H1 - the reason every handler below is wrapped.
 *
 * `buildProvider(...).invoke` in @office-ai/platform is
 * `new Promise(function(resolve){...})`: no reject, no timeout. The provider
 * half is `handler(data).then(cb)` with no `.catch`. So a throw in a handler
 * body is not an error the renderer can catch - it is a promise that NEVER
 * SETTLES. `TaskDetailPage.handleRunNow` sets `runningNow(true)`, awaits, and
 * neither its `catch` nor its `finally` ever runs: the button spins forever and
 * the three-option workspace message is shown to nobody.
 *
 * P2-10 made that reachable by ordinary use - rename the task folder in Finder,
 * press Run now - and P2-2 made it reachable from the enable toggle, because a
 * workspace allocation into a TCC-protected Documents path throws.
 *
 * Same shape as `speechToTextBridge`, which is where this pattern already
 * works: classify, resolve, render. Never throw across the seam.
 */
const cronFailure = (error: unknown): ICronBridgeFailure => {
  const classified = asCronWorkspaceError(error);
  if (classified) {
    return {
      ok: false,
  ...
  // The catch-all. The message is the same string the renderer already showed
  // via `Message.error(String(err))` before this wrapper existed, so nothing
  // new is disclosed - it just arrives now instead of never.
  return {
    ok: false,
...
/**
 * Wrap a provider body so a rejection becomes a RESOLVED failure payload.
 *
 * Applied to every provider in this file rather than only the three that can
 * hit a workspace problem: the guarantee that has to hold is "nothing on this
 * namespace can reject", and a per-handler judgement about which body might
 * throw is exactly how the three hangs already shipped got in.
 */
const guarded =
  <P, R>(handler: (params: P) => Promise<R>) =>
  async (params: P): Promise<R | ICronBridgeFailure> => {
    try {
      return await handler(params);
    } catch (error) {
      console.error('[cronBridge] provider failed:', error);
      return cronFailure(error);
    }
  };
...
export function initCronBridge(): void {
  // Query handlers
  ipcBridge.cron.listJobs.provider(guarded(async () => cronService.listJobs()));

  ipcBridge.cron.listArchivedJobs.provider(guarded(async () => cronService.listArchivedJobs()));

  ipcBridge.cron.listJobsByConversation.provider(
    guarded(async ({ conversationId }) => cronService.listJobsByConversation(conversationId))
  );

  ipcBridge.cron.getJob.provider(guarded(async ({ jobId }) => cronService.getJob(jobId)));

  // CRUD handlers
  ipcBridge.cron.addJob.provider(guarded(async (params) => cronService.addJob(params)));

  ipcBridge.cron.updateJob.provider(
    guarded(async ({ jobId, updates, allowHighFrequency }) => cronService.updateJob(jobId, updates, allowHighFrequency))
  );

  ipcBridge.cron.removeJob.provider(guarded(async ({ jobId }) => cronService.removeJob(jobId)));

  ipcBridge.cron.restoreArchivedJob.provider(
    guarded(async ({ archiveId }) => cronService.restoreArchivedJob(archiveId))
  );

  ipcBridge.cron.runNow.provider(
    guarded(async ({ jobId }) => {
  ...
  // Skill management
  ipcBridge.cron.saveSkill.provider(
    guarded(async ({ jobId, content }) => {
      await writeRawCronSkillFile(jobId, content);
    })
  );

  ipcBridge.cron.hasSkill.provider(guarded(async ({ jobId }) => hasCronSkillFile(jobId)));
  ...
  //   - Prompt length cap
  ipcBridge.cron.confirmProposal.provider(
    guarded(async (params: { conversationId: string; msgId: string; action: 'accept' | 'edit' | 'cancel' }) => {
      const { conversationId, msgId, action } = params;
      ...
      const VALID_BACKENDS: ReadonlyArray<string> = ['claude', 'codex', 'gemini', 'fuigo', 'qwen', 'kimi'];
