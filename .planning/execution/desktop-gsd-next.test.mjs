...
function snapshot(root) {
  ...
  return paths
    .toSorted()
    .map((path) => [path.slice(root.length), statSync(path).mode, readFileSync(path).toString('base64')]);
}
...
test('unrelated checkpoint does not block an unlocked autonomous plan', () => {
  const root = temporaryRoot();
  const output = select(root, [plan('01-01', { autonomous: false }), plan('01-02')]);
  assert.deepEqual(
    output.candidate_plans.map((item) => item.plan_id),
    ['01-02']
  );
...
test('incomplete autonomous predecessor is selected while its successor remains blocked', () => {
  const root = temporaryRoot();
  const output = select(root, [plan('01-01'), plan('01-02', { dependencies: ['01-01'] })]);
  assert.deepEqual(
    output.candidate_plans.map((item) => item.plan_id),
    ['01-01']
  );
...
test('unknown dependencies and cycles fail closed', () => {
  expectSelectionError(() => computeEffectiveWaves([plan('01-01', { dependencies: ['01-99'] })]), 'UNKNOWN_DEPENDENCY');
  expectSelectionError(
    () =>
      computeEffectiveWaves([plan('01-01', { dependencies: ['01-02'] }), plan('01-02', { dependencies: ['01-01'] })]),
    'DEPENDENCY_CYCLE'
  );
});
...
test('exact files and ancestor directories conflict', () => {
  const seams = [];
  assert.deepEqual(
    pairConflicts(plan('01-01', { files: ['src/a.ts'] }), plan('01-02', { files: ['src/a.ts'] }), seams),
    ['path:src/a.ts']
  );
  assert.deepEqual(pairConflicts(plan('01-01', { files: ['src'] }), plan('01-02', { files: ['src/a.ts'] }), seams), [
    'path:src<->src/a.ts',
  ]);
});
...
test('repository ownership resolves symlink aliases and rejects symlink escapes', () => {
  ...
  expectSelectionError(() => select(root, [plan('01-01', { files: ['repo-root-alias'] })]), 'UNSAFE_OWNERSHIP_PATH');
});

test('ownerless autonomous plans fail closed', () => {
  const root = temporaryRoot();
  expectSelectionError(() => select(root, [plan('01-01', { files: [], authoritySeams: [] })]), 'MALFORMED_PLAN');
});
...
test('allowlisted external ownership remains conflict-safe when installation plans unlock', () => {
  ...
  const output = select(
    root,
    [plan('01-01', { files: [`${externalRoot}/bin/tool`] }), plan('01-02', { files: [`${externalRoot}/bin`] })],
...
test('wrong repository branch HEAD and dirty state fail before selection', () => {
  ...
  assert.deepEqual(
    selectNext(base).candidate_plans.map((candidate) => candidate.plan_id),
    ['01-01']
  );
...
test('CLI pass and failure are byte-for-byte read-only', () => {
  ...
  const pass = spawnSync(
    process.execPath,
    [SELECTOR, '--repo-root', root, '--expected-branch', 'test-branch', '--expected-head', git.head],
    { encoding: 'utf8' }
  );
  ...
  const fail = spawnSync(
    process.execPath,
    [SELECTOR, '--repo-root', root, '--expected-branch', 'test-branch', '--expected-head', '0'.repeat(40)],
    { encoding: 'utf8' }
  );
...
test('operational selection rejects dangling and registered stale worktree paths', () => {
  ...
  expectSelectionError(() => selectNext({ ...base, worktreeParent: danglingParent }), 'WORKTREE_COLLISION');
  ...
  expectSelectionError(() => selectNext({ ...base, worktreeParent: registeredParent }), 'WORKTREE_COLLISION');
});
...
test('exact-HEAD discovery is NUL-safe for unusual phase directory path bytes', () => {
  ...
  const git = initializeGit(root);
  assert.deepEqual(
    discoverPlans(root, { gitHead: git.head }).map((item) => item.id),
    ['01-01']
  );
});
