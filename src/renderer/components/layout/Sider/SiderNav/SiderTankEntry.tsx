...
const SiderTankEntry: React.FC<SiderTankEntryProps> = ({ isActive, collapsed, siderTooltipProps, onClick }) => {
  const label = 'Tank';
    ...
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        <div
          ...
          onClick={onClick}
          data-testid='sider-tank-entry'
        >
          <Rocket size={16} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-20px h-20px flex items-center justify-center shrink-0'>
          <Rocket size={16} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
