...
function mockTokenOk(): void {
  mockFetch.mockResolvedValueOnce(
    new Response(JSON.stringify({ access_token: 'valid-bearer-token', expires_in: 3600 }), {
      status: 200,
...
describe('MsTeamsPlugin.testConnection', () => {
  ...
  it('returns success with botUsername = appId on valid credentials', async () => {
    ...
    const result = await MsTeamsPlugin.testConnection(JSON.stringify({ appId: 'my-app-id', appPassword: 'my-secret' }));
  ...
  it('returns error when appId is missing', async () => {
    const result = await MsTeamsPlugin.testConnection(JSON.stringify({ appId: '', appPassword: 'secret' }));
    expect(result.success).toBe(false);
    expect(result.error).toMatch(/appId/i);
  });

  it('returns error when appPassword is missing', async () => {
    const result = await MsTeamsPlugin.testConnection(JSON.stringify({ appId: 'my-app-id', appPassword: '' }));
    expect(result.success).toBe(false);
  ...
  it('returns auth error on 401 from Azure AD', async () => {
    ...
    const result = await MsTeamsPlugin.testConnection(JSON.stringify({ appId: 'bad-id', appPassword: 'wrong-secret' }));
