...
describe('OnboardingOverlay', () => {
  ...
  it('renders the scenario B hero "Connect Flux Router" CTA on a fresh machine', async () => {
    // Nothing detected ⇒ scenario B; flag unset ⇒ overlay shows.
    ...
    await waitFor(() => {
      expect(screen.getByText('onboarding.scenarioB.heroCta')).toBeInTheDocument();
    });
  });
  ...
  it('renders nothing when onboarding was already completed', async () => {
    ...
    expect(screen.queryByText('onboarding.scenarioB.heroCta')).not.toBeInTheDocument();
