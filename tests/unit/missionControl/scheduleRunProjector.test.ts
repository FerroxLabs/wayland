...
describe('projectScheduleRuns', () => {
  it('does not let a schedule definition or model claim mint a receipt', () => {
    ...
    expect(run.receipt).toEqual({ status: 'unavailable', reason: 'no receipt authority is available for this run' });
  });
  ...
  it('orders multiple runs newest-first', () => {
    ...
    expect(runs[1].receipt).toMatchObject({ status: 'unavailable' });
  });
