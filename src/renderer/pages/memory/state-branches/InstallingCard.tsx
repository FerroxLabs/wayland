...
const InstallingCard: React.FC<InstallingCardProps> = ({ version }) => {
  ...
  return (
    <div data-testid='memory-installing-card'>
      {degraded ? (
        <div className={styles.banner} data-testid='memory-installing-degraded-banner' role='status' aria-live='polite'>
          {t('degraded.banner')}
        </div>
      ) : null}
      <div className={styles.card} role='status' aria-live='polite' aria-busy={pendingActivation ? 'false' : 'true'}>
        <Spin size={32} />
