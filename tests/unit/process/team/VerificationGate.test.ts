...
import { clearIjfwGuardEnv } from '@process/utils/ijfwGuard';
...
beforeEach(() => {
  // Clear the guard's whole env surface, not a hand-picked subset. Listing
  // WAYLAND_DISABLE_IJFW and CI by hand left GITHUB_ACTIONS set, so every gate
  // call short-circuited to advisory on GitHub Actions while passing locally.
  clearIjfwGuardEnv(process.env);
});
