...
const SkillsHubSettings: React.FC<SkillsHubSettingsProps> = ({ withWrapper = true }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.show({
        variant: 'error',
        title: t('settings.skillsHub.fetchError', { defaultValue: 'Failed to fetch skills' }),
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleImport = async (skillPath: string) => {
    try {
      ...
      if (result.success) {
        toast.show({
          variant: 'success',
          title: result.msg || t('settings.skillsHub.importSuccess', { defaultValue: 'Skill imported successfully' }),
        });
        void fetchData();
      } else {
        toast.show({
          variant: 'error',
          title: result.msg || t('settings.skillsHub.importFailed', { defaultValue: 'Failed to import skill' }),
        });
      }
    } catch (error) {
      ...
      toast.show({
        variant: 'error',
        title: t('settings.skillsHub.importError', { defaultValue: 'Error importing skill' }),
      });
    }
  };
  ...
  const handleImportAll = async (skills: Array<{ name: string; path: string }>) => {
    ...
    if (successCount > 0) {
      toast.show({
        variant: 'success',
        title: t('settings.skillsHub.importAllSuccess', {
          count: successCount,
  ...
  const handleDelete = async (skillName: string) => {
    try {
      ...
      if (result.success) {
        toast.show({
          variant: 'success',
          title: result.msg || t('settings.skillsHub.deleteSuccess', { defaultValue: 'Skill deleted' }),
        });
        void fetchData();
      } else {
        toast.show({
          variant: 'error',
          title: result.msg || t('settings.skillsHub.deleteFailed', { defaultValue: 'Failed to delete skill' }),
        });
      }
    } catch (error) {
      ...
      toast.show({
        variant: 'error',
        title: t('settings.skillsHub.deleteError', { defaultValue: 'Error deleting skill' }),
      });
    }
  };
          ...
          <div className='px-[16px] md:px-[32px] py-32px bg-[var(--color-bg-2)] rd-12px mb-16px border-2 border-solid border-[var(--color-border-2)] relative overflow-hidden transition-all'>
            ...
            <div className='flex flex-col lg:flex-row lg:items-start justify-between gap-16px mb-24px relative z-10 w-full'>
              ...
              <div className='relative group shrink-0 w-full lg:w-[240px]'>
                ...
                <input
                  type='text'
                  className='w-full bg-fill-1 hover:bg-fill-2 border border-1 focus:border-primary-5 focus:bg-base outline-none rd-8px py-6px pl-36px pr-12px text-13px text-t-primary placeholder:text-t-tertiary transition-all shadow-sm box-border m-0'
            ...
            <div className='flex flex-wrap items-center gap-8px mb-20px relative z-10 w-full'>
              {externalSources.map((source) => {
                ...
                return (
                  <button
                    key={source.source}
                    type='button'
                    className={`outline-none cursor-pointer px-16px py-6px text-13px rd-[100px] transition-all duration-300 flex items-center gap-6px border ${isActive ? 'bg-primary-6 border-primary-6 text-white shadow-md font-medium' : 'bg-base border-1 text-t-secondary hover:bg-fill-1 hover:text-t-primary'}`}
              ...
              <button
                type='button'
                className='outline-none border border-dashed border-1 hover:border-primary-4 cursor-pointer w-28px h-28px ml-4px text-t-tertiary hover:text-primary-6 hover:bg-primary-1 rd-full transition-all duration-300 flex items-center justify-center bg-transparent shrink-0'
                ...
                <div className='max-h-[360px] overflow-y-auto custom-scrollbar flex flex-col gap-6px pr-4px'>
                  {filteredExternalSkills.map((skill) => (
                    <div
                      ...
                      className={`group flex flex-col sm:flex-row gap-16px p-16px bg-base border hover:border-1 hover:bg-fill-1 hover:shadow-sm rd-12px transition-all duration-200 cursor-pointer ${highlightedSkill === skill.name ? 'border-primary-5 bg-primary-1' : 'border-transparent'}`}
                      onClick={() => void handleImport(skill.path)}
                    >
                      <div className='shrink-0 flex items-start sm:mt-2px'>
                        <div className='w-40px h-40px rd-full bg-base border border-1 flex items-center justify-center font-bold text-16px text-t-primary shadow-sm transition-all text-transform-uppercase'>
                          {skill.name.charAt(0)}
                        </div>
                      </div>
        ...
        <div className='px-[16px] md:px-[32px] py-32px bg-[var(--color-bg-2)] rd-12px border-2 border-solid border-[var(--color-border-2)] relative overflow-hidden transition-all'>
          ...
          <div className='flex flex-col lg:flex-row lg:items-center justify-between gap-16px mb-24px relative z-10'>
            ...
            <div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-12px w-full lg:w-auto shrink-0'>
              ...
              <button
                className='flex items-center justify-center gap-6px px-16px py-6px bg-base border border-1 hover:border-2 hover:bg-fill-1 text-t-primary rd-8px shadow-sm transition-all focus:outline-none shrink-0 cursor-pointer whitespace-nowrap'
            ...
            <div className='w-full flex flex-col gap-6px relative z-10'>
              {filteredSkills.map((skill) => (
                <div
                  ...
                  className={`group flex flex-col sm:flex-row gap-16px p-16px bg-base border hover:border-1 hover:bg-fill-1 hover:shadow-sm rd-12px transition-all duration-200 ${highlightedSkill === skill.name ? 'border-primary-5 bg-primary-1' : 'border-transparent'}`}
                >
                  <div className='shrink-0 flex items-start sm:mt-2px'>
                    <div
                                ...
                                onClick={async (e) => {
                                  ...
                                  try {
                                    ...
                                    if (result.success) {
                                      toast.show({
                                        variant: 'success',
                                        title: t('settings.skillsHub.exportSuccess', {
