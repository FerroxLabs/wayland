...
4. `data_get_indicator` — read indicator values. It takes an `entity_id` from
   `chart_get_state`; a study reported with `id: null` and `addressable_by: "name"` has no
   server id, which means the pane is damaged — run `tv_chart_health` rather than working
   around it
...
1. `watchlist_get` — read all symbols. Membership is complete even with the panel closed, but
   prices are only attached when the panel is open: check `quotes_available` before reading a
   price off it
