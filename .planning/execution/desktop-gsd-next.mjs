  ...
  if (absolute) {
    if (!normalized.startsWith('/')) {
      throw new SelectionError(
        ...
        `External ownership path is not supported on this host: ${path}`
      );
    }
...
export function discoverPlans(repoRoot, options = {}) {
  ...
  const plans = [];
  const gitPaths = options.gitHead ? new Set(gitTreePaths(repoRoot, options.gitHead, '.planning/phases')) : null;
  const entries = gitPaths
    ? [...gitPaths]
        .filter((source) => {
          const parts = source.split('/');
          return (
            parts.length === 4 && parts[0] === '.planning' && parts[1] === 'phases' && parts[3].endsWith('-PLAN.md')
          );
        })
        .map((source) => ({
  ...
  for (const { phaseDir, name, path, source } of entries) {
    ...
    if (
      dependencies.some((dependency) => !/^\d{2}-\d{2}$/.test(dependency)) ||
...
function pairConflicts(left, right, seamPatterns, allowedExternalRoots = []) {
  ...
  for (const { name, regex } of seamPatterns) {
    if (
      left.files.some((path) => regex.test(canonicalOwnedPath(path, allowedExternalRoots))) &&
...
function validateAdmissionConfig(config, repoRoot) {
  ...
  if (
    ...
    !verifier.path.startsWith('/') ||
    !/^sha256:[0-9a-f]{64}$/.test(verifier.digest ?? '') ||
    !Number.isInteger(verifier.timeout_ms) ||
    verifier.timeout_ms < 100 ||
    verifier.timeout_ms > 120_000 ||
    !Number.isInteger(verifier.max_output_bytes) ||
    verifier.max_output_bytes < 1024 ||
    verifier.max_output_bytes > 10_485_760
  ) {
...
function everyPrerequisiteGreen(prerequisites) {
  ...
  return (
    items.length > 0 &&
    items.every((item) => item && typeof item === 'object' && !Array.isArray(item) && item.ok === true)
  );
}

export function validateEntryReceipt(receipt, expectedGate) {
  if (!receipt || receipt.schema_version !== 2)
    throw new SelectionError('VERIFIER_VERSION', 'Verifier output is not schema v2');
  if (receipt.gate_id !== expectedGate)
    throw new SelectionError('VERIFIER_GATE', `Verifier returned ${receipt.gate_id ?? 'no gate'} for ${expectedGate}`);
...
export function invokeVerifier(verifier, gateId, cwd, { afterSnapshot } = {}) {
  ...
  const result = spawnSync(
    process.execPath,
    ['--input-type=module', '--eval', `process.argv.splice(1, 0, 'wayland-gsd-gate');\n${source}`, gateId],
    {
      cwd,
      encoding: 'utf8',
...
function readGitRegularFile(repoRoot, head, source, code, label) {
  const entry =
    execFileSync('git', ['ls-tree', '-z', head, '--', source], {
      cwd: repoRoot,
      encoding: 'utf8',
    }).split('\0')[0] ?? '';
...
function selectNextInternal(options) {
  ...
  const admittedSelected = selected.map((plan) =>
    Object.assign({}, plan, {
